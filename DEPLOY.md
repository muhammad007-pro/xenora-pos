...
- **v1.12.9** (2026-09-11, `7134d4e`): POS sidebar'ida "Smena" havolasi + smena ochish/yopish ruxsat qatlami (`routers/shift.py`). Migratsiya YO'Q, alembic `b3d7f1c92a48` o'zgarmadi. Backup: `~/xenora-backups/pre_1.12.9_2026-09-10_1921.sql.gz` (16.8 MB, `gzip -t` OK). Rollback: `6575265`. Health `version:1.12.9` (lokal + app.xenora.uz). 5xx **0**, ERROR/CRITICAL **0**. Baseline (20/26/27) **aynan o'zgarmagan** (`diff` toza).
  ...
  - ⚠️ **Xulq o'zgarishi:** `POST /shifts/` endi boshqa `user_id` ni qabul qilmaydi (403), `POST /shifts/{id}/close` ni kassir faqat o'zinikiga qila oladi (`manage_shifts` — admin/menejer istalganiga). Prod audit: 11 smenadan 10 tasi xodim o'zi ochgan, ya'ni mavjud oqim buzilmaydi.
  - Tekshirilmagan: FAZZA/1001 BARAKA tenant login (kredensial yo'q). Deploydan keyin `shifts` endpointiga hali so'rov kelmadi (tinch payt).
- **v1.12.8** (2026-09-10, `f63f68b`): qoldiq keshi (frontend) + `pos-stock` N+1 (`joinedload`). Migratsiya YO'Q, alembic `b3d7f1c92a48` o'zgarmadi. Backup: `~/xenora-backups/pre_1.12.8_2026-09-10_1708.sql.gz` (16.7 MB, `gzip -t` OK). Rollback: `505494f`. Health `version:1.12.8` (lokal + app.xenora.uz). 5xx **0**, ERROR/CRITICAL **0**. Baseline (20/26/27) **aynan o'zgarmagan** (`diff` toza).
  - ✅ **`pos-stock` tezlashdi.** Prod ORM o'lchovi (tenant 27, 534 qator, 3 urinish): **390.9 ms → 21.6 ms (~18×)**. Jonli HTTP (auth+JSON bilan): deploydan oldin 426–767 ms (139 chaqiruv/6 soat), keyin **261.7 / 71.9 / 68.2 ms**.
  - ✅ **Javob shakli o'zgarmadi.** `git diff 505494f f63f68b -- routers/inventory.py` butun faylda faqat 2 ta funksional qator ko'rsatadi (import + query). Kalitlar: 10 ta asosiy (`product_id, name, price, sale_unit, quantity, unit, min_threshold, image_url, pack_size, pack_price`) + `view_finance` bo'lsa `cost_price`/`stock_value` va yuqori darajada `total_value`. Kontrakt testi: `backend/tests/test_pos_stock_contract.py`.
  - Log'dagi 404'lar — internet skaner botlari (`.env`, `app.config.js`, `phpunit`). Oldindan bor, to'g'ri 404 qaytadi. 422 (`promotions/gift-suggestions`) ham oldindan mavjud (24 soatda 156 marta) — alohida tekshirilishi kerak.
  - Tekshirilmagan: FAZZA/1001 BARAKA tenant login (kredensial yo'q).
- **v1.12.7** (2026-09-10, `ee4b942`): mahsulotlar ro'yxatida "Qoldiq" ustuni + kategoriya nomi tuzatildi. **FAQAT FRONTEND** — migratsiya YO'Q, alembic `b3d7f1c92a48` o'zgarmadi (upgrade no-op). Backup: `~/xenora-backups/pre_1.12.7_2026-09-10_1250.sql.gz` (16.5 MB, `gzip -t` OK). Rollback: `fa172f4`. Health `version:1.12.7` (lokal + app.xenora.uz). Deploydan keyin: 5xx **0**, ERROR/CRITICAL **0**. Baseline: eco aroma 555/17/16 va FAZZA 930/750/629 **o'zgarmagan**; 1001 BARAKA buyurtma 125→126, to'lov 112→113 — bu **jonli sotuv** (deploy paytida do'kon ishlayotgan edi, log'da order=920 + OFD cheki), yo'qotish EMAS; mahsulot/ombor 524/524 o'zgarmagan.
  - ⚠️ **`/inventory/pos-stock` sekin: 442–909 ms** (1001 BARAKA, 524 qator, jonli o'lchov). Sabab N+1: `routers/inventory.py:171` `db.query(Inventory).join(Product)` da `joinedload` YO'Q → `i.product` har qator uchun alohida SELECT. Asosiy SQL o'zi **2 ms** (`EXPLAIN ANALYZE`), bitta mahsulot lookup 0.17 ms × 524 ≈ qolgani. Yechim: `.options(joinedload(Inventory.product))`.
  - ⚠️ **Frontend kuchaytirishi:** `productsSearch` da debounce yo'q — har harf `loadProducts()` chaqiradi, endi u pos-stock ni ham tortadi. Qoldiq xaritasi qidiruvga bog'liq emas → bir marta olinib keshlanishi kerak.
  - Tekshirilmagan: FAZZA/1001 BARAKA tenant login (kredensial yo'q). `/auth/login` **form-data** kutadi (`OAuth2PasswordRequestForm`), JSON EMAS — 401 to'g'ri qaytdi (endpoint sog'lom).
