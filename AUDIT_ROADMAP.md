...
- [x] Mijoz/web backup 401 tuzatish — BAJARILDI (api.js fetchBinary + refresh, tenant-backup.js ulandi, oxirgi backup sanasi + 48h ogohlantirish). Deploy kutmoqda.
- [ ] Media (rasm) backup — pg_dump'ga qo'shimcha, uploads papkasi ham zaxiralanadi
- [x] SVG upload'ni o'chirish — BAJARILDI (.svg va .gif rad, PIL magic-byte tekshiruvi, nginx nosniff statik location'ga). Deploy kutmoqda.
...
- [x] /customers/all ni cheklash — BAJARILDI (default 500, max 2000, frontend buzilmadi). Deploy kutmoqda.

### TIER 1 — Daromad himoyasi (biznes-kritik)
- [ ] /subscription/upgrade + /renew — faqat superadmin, IDOR yopish, to'lovga bog'lash
- [ ] Per-request obuna enforcement (dependency: expires<now yoki status blocked → 402/403 + grace period)
...
- [ ] Race condition qulfi (with_for_update)
- [ ] Ombor ayirishni transaction ichiga olish (atomik)
- [ ] Multi-commit atomiklik + get_db rollback
